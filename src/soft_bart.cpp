...
// TODO:
// 1. Initialize s and is_sine in model (DONE)
// 2. Add update for s and is_sine -> randomly select
// 3. Add phi (the length scale) as a hyperparameter (DONE)
// 4. Add hyperprior for phi (DONE)
// 5. Initialize phi and its hyperprior (shape_phi, rate_phi) (DONE)
// 6. Add an update for phi and derive its posterior (DONE)
// 7. Write basis function eval (DONE)
// 8. Add targeting variable t to all the function arguments

#define C_LOG2PI 1.83787706640934548356

using namespace Rcpp;
using namespace arma;

double EvalBasisFunction(Node* n, double t) {
  // return n->is_sine ? sin(t * n->frequency) : cos(t * n->frequency);
  return M_SQRT2 * cos(t * n->frequency + n->offset);
}

void UpdateFrequency(Node* n, const arma::vec& Y, const arma::vec& t, const arma::mat& X, const Hypers& hypers)
{
  
  double frequency_old = n->frequency;
  double frequency_new = norm_rand() / hypers.length_scale;
  // double frequency_new = pow(2. * M_PI * hypers.length_scale, -0.5) * norm_rand();
  double offset_old = n->offset;
  double offset_new = unif_rand() * 2. * M_PI;
  
  // bool is_sine_old = n->is_sine;
  // bool is_sine_new = unif_rand() < 0.5 ? true : false;

  double loglik_old = LogLT(n, Y, t, X, hypers);

  n->frequency = frequency_new;
  n->offset = offset_new;
  // n->is_sine = is_sine_new;

  double loglik_new = LogLT(n, Y, t, X, hypers);

  bool accept = log(unif_rand()) < loglik_new - loglik_old ? true : false;
  if(!accept) {
    n->frequency = frequency_old;
    n->offset = offset_old;
  }
  // else {
  //   Rcout << "Accepted!";
...
void UpdateLengthScale(Hypers& hypers, std::vector<Node*>& forest) {
  double shape_up = hypers.shape_length_scale + 0.5 * forest.size();
  double rate_up  = hypers.rate_length_scale;
  for(int i = 0; i < forest.size(); i++) {
    rate_up += 0.5 * pow(forest[i]->frequency, 2);
  }
  hypers.length_scale = sqrt(R::rgamma(shape_up, 1.0 / rate_up));
}
...
void Hypers::UpdateSigmaMu(const arma::vec& means) {
  sigma_mu = update_sigma_halfcauchy_trunc(means, sigma_mu_hat, sigma_mu, temperature, num_tree);
}
...
Hypers::Hypers(Rcpp::List hypers)
{
  ...
  shape_length_scale = hypers["shape_length_scale"];
  rate_length_scale = hypers["rate_length_scale"];
...
Node::Node(const Hypers& hypers) {
  ...
  // Basis Function
  // frequency = pow(2. * M_PI * hypers.length_scale, -0.5) * norm_rand();
  frequency = norm_rand() / hypers.length_scale;
  offset = 2. * M_PI * unif_rand();
  // is_sine = unif_rand() < 0.5 ? true : false;
...
void Node::UpdateMu(const arma::vec& Y, const arma::vec& t, const arma::mat& X, const Hypers& hypers) {

  std::vector<Node*> leafs = leaves(this);
  int num_leaves = leafs.size();

  // Get mean and covariance
  vec Z = zeros<vec>(num_leaves);
  mat A = zeros<mat>(num_leaves, num_leaves);
  GetSuffStats(this, Y, t, X, hypers, Z, A);

  mat Vinv = A * pow(hypers.sigma,-2) + eye(num_leaves, num_leaves) * pow(hypers.sigma_mu,-2);
  mat V = inv_sympd(Vinv);
  vec m = solve(Vinv, Z) * pow(hypers.sigma,-2);

  // vec mu_samp = rmvnorm(m, Vinv);
  vec mu_samp = mvnrnd(m, V);
...
void Node::UpdateTau(const arma::vec& Y,
  ...
  double loglik_new = loglik_tau(tau_new, X, Y, t, hypers) + logprior_tau(tau_new, hypers.tau_rate);
  double loglik_old = loglik_tau(tau_old, X, Y, t, hypers) + logprior_tau(tau_old, hypers.tau_rate);
...
double Node::loglik_tau(double tau_new, const arma::mat& X,
  ...
  double tau_old = tau;
  SetTau(tau_new);
  double out = LogLT(this, Y, t, X, hypers);
  SetTau(tau_old);
  return out;
...
arma::vec predict(const std::vector<Node*>& forest,
  ...
  for(int tt = 0; tt < num_tree; tt++) {
    out = out + predict(forest[tt], X, t, hypers);
  }

  return out;
}

arma::vec predict(Node* n, const arma::mat& X, const arma::vec& t, const Hypers& hypers) {
  ...
  for(int i = 0; i < N; i++) {
    ...
    for(int j = 0; j < num_leaves; j++) {
      out(i) = out(i) + leafs[j]->current_weight * leafs[j]->mu * EvalBasisFunction(n,t(i));
    }
  }
...
/*
Computes the sufficient statistics A and Z described in the paper. Z is equal to
Psi^T Y while A is equal to Psi^T Psi, where Psi is a matrix of basis function evaluations.
 */
void GetSuffStats(Node* n, const arma::vec& y, const arma::vec& t,
                  const arma::mat& X, const Hypers& hypers,
                  arma::vec& Z, arma::mat& A) {

  int num_obs = y.size();
  std::vector<Node*> leafs = leaves(n);
  int num_leaves = leafs.size();
  mat Psi = zeros<mat>(num_obs, num_leaves);

  for(int i = 0; i < X.n_rows; i++) {
    n->GetW(X, i);
    for(int j = 0; j < num_leaves; j++) {
      Psi(i,j) = leafs[j]->current_weight * EvalBasisFunction(n,t(i));
    }
  }
...
double LogLT(Node* n, const arma::vec& Y, const arma::vec& t,
             const arma::mat& X, const Hypers& hypers) {
  ...
  arma::vec Z = zeros<vec>(num_leaves);
  arma::mat A = zeros<mat>(num_leaves, num_leaves);
  GetSuffStats(n, Y, t, X, hypers, Z, A);

  int N = Y.size();
  double ld_val; double ld_sign;
  double sigsq = pow(hypers.sigma,2);
  double tausq = pow(hypers.sigma_mu,2);

  mat B = eye(num_leaves, num_leaves) + tausq / sigsq * A;
  mat Binv = inv_sympd(B);
  
  log_det(ld_val, ld_sign, B);

  double out = -0.5 * N * C_LOG2PI;
  out -= 0.5 * N * log(sigsq); 
  out -= 0.5 * ld_val;
  out -= 0.5 / sigsq * (sum(Y % Y) - tausq / sigsq * as_scalar(Z.t() * Binv * Z));
...
void birth_death(Node* tree, const arma::mat& X, const arma::vec& Y,
  ...
  if(unif_rand() < p_birth) {
    node_birth(tree, X, Y, t, hypers);
  }
  else {
    node_death(tree, X, Y, t, hypers);
  }
}
...
void node_birth(Node* tree, const arma::mat& X, const arma::vec& Y,
  ...
  double ll_before = LogLT(tree, Y, t, X, hypers);
  ...
  double ll_after = LogLT(tree, Y, t, X, hypers);
...
void node_death(Node* tree, const arma::mat& X, const arma::vec& Y,
  ...
  double ll_after = LogLT(tree, Y, t, X, hypers) + log(1.0 - leaf_prob);
...
Node* draw_prior(Node* tree, const arma::mat& X, const arma::vec& Y, const arma::vec& t, const Hypers& hypers) {

  // Compute loglik before
  Node* tree_0 = tree;
  // Rcout << "A";
  double loglik_before = LogLT(tree_0, Y, t, X, hypers);
  ...
  // Rcout << "B";
  double loglik_after = LogLT(tree_1, Y, t, X, hypers);
  // Rcout << "Calc loglik" << "\n";
...
void TreeBackfit(std::vector<Node*>& forest, arma::vec& Y_hat,
  ...
  for(int m = 0; m < num_tree; m++) {
    // Rcout << "Getting backfit quantities";
    arma::vec Y_star = Y_hat - predict(forest[m], X, t, hypers);
    arma::vec res = Y - Y_star;

    if(unif_rand() < MH_PRIOR) {
      // Rcout << "Draw Prior";
      forest[m] = draw_prior(forest[m], X, res, t, hypers);
      // Rcout << "Done";
    }
    if(forest[m]->is_leaf || unif_rand() < MH_BD) {
      // Rcout << "BD step";
      birth_death(forest[m], X, res, t, hypers);
      // Rcout << "Done";
    }
    else {
      // Rcout << "Change step";
      perturb_decision_rule(forest[m], X, res, t, hypers);
      // Rcout << "Done";
    }
    if(opts.update_tau) forest[m]->UpdateTau(res, t, X, hypers);
    if(opts.update_tau) UpdateFrequency(forest[m], res, t, X, hypers);
    forest[m]->UpdateMu(res, t, X, hypers);
    Y_hat = Y_star + predict(forest[m], X, t, hypers);
  }
}
...
void Forest::IterateGibbs(arma::vec& Y_hat,
  ...
  // Rcout << "Backfitting";
  TreeBackfit(trees, Y_hat, hypers, X, Y, t, opts);
  ...
  UpdateLengthScale(hypers, trees);
...
arma::mat Forest::do_gibbs(const arma::mat& X,
  ...
  vec Y_hat = predict(trees, X, t, hypers);
  mat Y_out = zeros<mat>(num_iter, X_test.n_rows);

  for(int i = 0; i < num_iter; i++) {
    IterateGibbs(Y_hat, X, Y, t);
    vec tmp = predict(trees, X_test, t_test, hypers);
    Y_out.row(i) = tmp.t();
    if((i+1) % opts.num_print == 0) {
...
arma::vec Forest::predict_vec(const arma::mat& X_test, const arma::vec& t_test) {
  return predict(trees, X_test, t_test, hypers);
}
...
Rcpp::List Forest::get_params() {
  ...
  out["length_scale"] = hypers.length_scale;
  out["s"] = hypers.s;
