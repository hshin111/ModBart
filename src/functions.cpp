...
arma::vec rmvnorm_eig(const arma::vec& mean, const arma::mat& Precision) {
  ...
  arma::vec eigval;
  arma::mat eigvec;
  eig_sym(eigval, eigvec, Precision);

  z = eigvec * z;
  for(int i = 0; i < mean.size(); i++) {
    z(i) = z(i) / sqrt(eigval(i));
  }
...
double update_sigma_halfcauchy_trunc(const arma::vec& r, double sigma_hat, double sigma_old,
                    double temperature, double num_tree) {
  ...
  return log(unif_rand()) < loglik_rat ? fmin(sigma_prop, 3.5/(sqrt(num_tree)*2)) : sigma_old;
